#!/usr/bin/env python3
"""Portable, resumable TAPS GROMACS runner.

Works after the folder is copied anywhere (laptop or server). All paths are
resolved from this file's location, not from the current working directory
and not from any machine-specific absolute path.

Paper mainline (copy this folder to the server, then):
  python3 run_md.py --check                 # Python packages, gmx, input files
  python3 run_md.py --length 100            # all CPU threads, GPU offload, live progress
  python3 run_md.py --length 200
  python3 run_md.py --length 500

Examples
--------
  python3 run_md.py --system ala2_vacuum --length 100
  python3 run_md.py --nt 16                 # cap threads (default: all logical CPUs)
  python3 run_md.py --short                 # extra 100 ps TAPS smoke-test trajectory
  python3 run_md.py --cpu                   # disable GPU
  python3 run_md.py --force                 # ignore previous results and rerun
"""
...
PROD_LENGTHS_NS = (100, 200, 500)
DT_PS = 0.002

# MD runner uses the Python standard library only. These are for later TAPS work.
OPTIONAL_PACKAGES = (
    ("numpy", "numpy", "analysis / later TAPS"),
    ("scipy", "scipy", "density / MSM helpers later"),
    ("torch", "torch", "TAPS temporal model later"),
)
...
SYSTEMS: Dict[str, SystemSpec] = {
    "ala2_vacuum": SystemSpec(
        key="ala2_vacuum",
        title="Alanine dipeptide vacuum",
        rel_dir="systems/alanine_dipeptide/vacuum",
        solvent="vacuum",
        em_input="box.gro",
        em_mdp="em_vacuum.mdp",
        nvt_mdp="nvt_vacuum.mdp",
        npt_mdp=None,
        short_mdp="md_short_vacuum.mdp",
        ref_mdp="md_ref_vacuum.mdp",
        required_mdp=(
            "em_vacuum.mdp",
            "nvt_vacuum.mdp",
            "md_short_vacuum.mdp",
    ...
    "ala2_water": SystemSpec(
        key="ala2_water",
        title="Alanine dipeptide water",
        ...
        ref_mdp="md_ref.mdp",
        required_mdp=("em.mdp", "nvt.mdp", "npt.mdp", "md_short.mdp", "md_ref.mdp"),
    ),
    "ala4_vacuum": SystemSpec(
        key="ala4_vacuum",
        title="Alanine tetrapeptide vacuum",
        rel_dir="systems/alanine_tetrapeptide/vacuum",
        solvent="vacuum",
        em_input="box.gro",
        em_mdp="em_vacuum.mdp",
        nvt_mdp="nvt_vacuum.mdp",
    ...
    "ala4_water": SystemSpec(
        key="ala4_water",
        title="Alanine tetrapeptide water",
        rel_dir="systems/alanine_tetrapeptide/water",
        solvent="water",
        em_input="ions.gro",
        em_mdp="em.mdp",
        nvt_mdp="nvt.mdp",
    ...
    "cln025_water": SystemSpec(
        key="cln025_water",
        title="Chignolin CLN025 water (native/folded start)",
        rel_dir="systems/chignolin_cln025/water",
        solvent="water",
        em_input="ions.gro",
        em_mdp="em.mdp",
        nvt_mdp="nvt.mdp",
    ...
    "cln025_unfolded": SystemSpec(
        key="cln025_unfolded",
        title="Chignolin CLN025 water (unfolded start)",
        rel_dir="systems/chignolin_cln025/water_unfolded",
        solvent="water",
        em_input="ions.gro",
        em_mdp="em.mdp",
        nvt_mdp="nvt.mdp",
    ...
    "adk_open": SystemSpec(
        key="adk_open",
        title="AdK open (optional)",
        rel_dir="systems/adk/water_open",
        solvent="water",
        em_input="ions.gro",
        em_mdp="em.mdp",
        nvt_mdp="nvt.mdp",
    ...
    "adk_closed": SystemSpec(
        key="adk_closed",
        title="AdK closed (optional)",
        rel_dir="systems/adk/water_closed",
        solvent="water",
        em_input="ions.gro",
        em_mdp="em.mdp",
        nvt_mdp="nvt.mdp",
...
def check_python_packages() -> List[str]:
    ...
    if missing:
        log("later TAPS: python3 -m pip install " + " ".join(missing), "WARN")
    else:
        log("optional TAPS packages: all present")
...
def self_check(gmx: str, specs: Sequence[SystemSpec], repair: bool = True) -> List[str]:
    ...
    for tmpl in ("md_prod.mdp", "md_prod_vacuum.mdp"):
    ...
    if gro_in.exists():
        ...
        if natom < 500 and gpu:
            log(
                ...
                "that is normal. Water / CLN025 / AdK load the GPU much more.",
                "WARN",
            )
    results = {}
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Self-checking, resumable GROMACS runner for the TAPS folder.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Paper mainline: EM -> 1 ns NVT -> (1 ns NPT) -> 100/200/500 ns production.

Default systems: ala2_vacuum, ala2_water, ala4_vacuum, ala4_water, cln025_water
Unfolded CLN025: --system cln025_unfolded
AdK is skipped unless prepared and selected with --system / --adk.

GPU is auto-detected (nvidia-smi). Default mdrun uses all CPU threads,
thread pinning, and full GPU offload (nb/pme/bonded/update). Use --cpu
to disable GPU, --gpu to force it, --nt N to cap threads.

A live progress line is printed during mdrun (percent, ns, ns/day, GPU, ETA).

Resume: rerun the same command. Finished steps are skipped; an interrupted
mdrun is continued from its .cpt checkpoint. Different --length values write
separate files (runs/md_100ns, runs/md_200ns, runs/md_500ns).
""",
    )
    p.add_argument(
        "--system",
    ...
    p.add_argument("--adk", action="store_true", help="include AdK if water_open/closed exist")
    p.add_argument(
        "--length",
        type=int,
        default=100,
        choices=[0, *PROD_LENGTHS_NS],
        help="unrestrained production length in ns (0 = skip production). default: 100",
    )
    p.add_argument(
        "--short",
        action="store_true",
        help="also run a 100 ps short trajectory (TAPS adaptive-round smoke test)",
    )
    p.add_argument(
        "--short-500ps",
        action="store_true",
...
def select_specs(args: argparse.Namespace) -> List[SystemSpec]:
    if args.systems:
        keys = args.systems
        if args.adk:
            keys = list(keys) + [k for k in ("adk_open", "adk_closed") if k not in keys]
    else:
        keys = list(DEFAULT_KEYS)
        if args.adk:
            keys += ["adk_open", "adk_closed"]
    specs = []
    unknown = []
    for key in keys:
        ...
        if not spec.default and not spec.workdir.is_dir():
            log(f"skip {key}: not prepared (run INCLUDE_ADK=1 bash scripts/prepare_gromacs.sh)", "WARN")
            continue
        specs.append(spec)
    if unknown:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    length = args.length
    if args.ref:
        log("--ref is deprecated; production length is --length (default 100 ns)", "WARN")
        if length == 0:
            length = 100
    ...
    if length and len(specs) > 1:
        log(
            ...
            "paper mainline: start with --system ala2_vacuum --length 100",
            "WARN",
        )
    if args.check:
    ...
    if length == 0 and not do_short:
        log("nothing to produce (--length 0 and no --short)", "ERROR")
        log("use --length 100 (default) or add --short", "ERROR")
        return 2
