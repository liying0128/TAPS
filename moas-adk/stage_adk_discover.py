#!/usr/bin/env python3
"""AdK open→closed adaptive sampling. Four methods only: Random, LAST, LC, MOAS-static.

CVs are LID–CORE and NMP–CORE angles (degrees). Success is both angles inside
the closed window for ≥ 200 ps (not first-hit).
...
Requires equilibrated open box and an init cMD (20 ns prefix of a 200 ns campaign):
  ...
  python3 stage_adk_discover.py --gpu --nt 16 --methods random last density moas \
...
from adk_angles import angles_from_ca, load_refs  # noqa: E402


def in_closed(lid, nmp, refs: dict) -> np.ndarray:
    lid = np.asarray(lid)
    nmp = np.asarray(nmp)
    lid_ok = lid < refs["closed_lid_thr"] if refs["closed_lid_op"] == "lt" else lid > refs["closed_lid_thr"]
    nmp_ok = nmp < refs["closed_nmp_thr"] if refs["closed_nmp_op"] == "lt" else nmp > refs["closed_nmp_thr"]
    return lid_ok & nmp_ok
from adk_common import (  # noqa: E402
    ANALYSIS,
    AdkError,
    dump_frame,
    find_gmx,
...
class AdkSpec:
    ...
    def outdir(self) -> Path:
        return ANALYSIS / "adk_open" / "campaigns" / self.tag
...
def mdrun_short(gmx: str, workdir: Path, deffnm: str, nt: int, gpu: bool) -> None:
    ...
    if not logp.exists() or "Finished mdrun" not in logp.read_text(encoding="utf-8", errors="replace"):
        raise AdkError(f"short MD did not finish: {logp}")


def extract_angles(tpr: Path, xtc: Path, dest_npz: Path, scratch: Path) -> dict:
    ...
    if xyz.shape[1] != len(resids):
        raise AdkError(f"CA count {xyz.shape[1]} != gro {len(resids)}")
    lid, nmp = angles_from_ca(xyz, resids)
    refs = load_refs()
    closed = in_closed(lid, nmp, refs)
    np.savez_compressed(dest_npz, t_ps=t_ps, lid=lid, nmp=nmp, closed=closed.astype(np.int8))
    return {"t_ps": t_ps, "lid": lid, "nmp": nmp, "closed": closed}


def to_bins(lid, nmp, nbins, lo, hi):
...
def density_at(lid_all, nmp_all, lid_q, nmp_q, nbins, lo, hi):
    i, j = to_bins(lid_all, nmp_all, nbins, lo, hi)
    hist = np.zeros((nbins, nbins), dtype=np.float64)
    np.add.at(hist, (i, j), 1)
    hist /= max(1.0, hist.sum())
    iq, jq = to_bins(lid_q, nmp_q, nbins, lo, hi)
    return hist[iq, jq]


def last_frontier_scores(lid_all, nmp_all, lid_q, nmp_q, nbins: int = 24) -> np.ndarray:
...
def greedy_diverse(lid, nmp, score, n_seeds: int, min_deg: float) -> list:
    ...
    for idx in order:
        i = int(idx)
        if all(float(np.hypot(lid[i] - lid[j], nmp[i] - nmp[j])) >= min_deg for j in picked):
            picked.append(i)
...
def concat_pool(segments: list) -> dict:
    lid = np.concatenate([s["lid"] for s in segments])
    nmp = np.concatenate([s["nmp"] for s in segments])
    ...
    return {"lid": lid, "nmp": nmp, "closed": closed, "t_accum_ps": t_accum, "sim_ns": clock / 1000.0}
...
def snapshot(segments, refs, nbins: int) -> dict:
    ...
    lid, nmp = pool["lid"], pool["nmp"]
    lo = np.array([min(lid.min(), refs["closed_lid"] - 5), min(nmp.min(), refs["closed_nmp"] - 5)])
    hi = np.array([max(lid.max(), refs["open_lid"] + 5), max(nmp.max(), refs["open_nmp"] + 5)])
    i, j = to_bins(lid, nmp, nbins, lo, hi)
    ...
    return {
        ...
        "mean_nmp": float(nmp.mean()),
        "min_lid": float(lid.min()),
        "min_nmp": float(nmp.min()),
    }
...
def ensure_cmd_cvs(force: bool = False) -> dict:
    dest = ANALYSIS / "adk_open" / "cvs.npz"
    if dest.exists() and not force:
        log(f"reuse {dest.relative_to(ROOT)}")
        data = np.load(dest)
        return {k: data[k] for k in data.files}
    if not PROD_XTC.exists() or not PROD_TPR.exists():
        raise AdkError(f"missing init cMD {PROD_XTC}. Run: python3 run_md.py --system adk_open --length 20 --gpu")
    log("extracting LID/NMP angles from open 20 ns cMD")
    return extract_angles(PROD_TPR, PROD_XTC, dest, ANALYSIS / "adk_open" / "scratch_cmd")


def slice_cmd_init(init_ns: float) -> dict:
    data = np.load(ANALYSIS / "adk_open" / "cvs.npz")
...
def rebuild_segments(spec: AdkSpec, init_ns: float) -> list:
...
def select_and_dump(spec, pack, method, n_seeds, min_deg, segments, seed_dir: Path, rng_seed: int, refs: dict) -> list:
    lid_e, nmp_e = pack["lid_end"], pack["nmp_end"]
    pool = concat_pool(segments)
    lo = np.array([pool["lid"].min() - 2, pool["nmp"].min() - 2])
    hi = np.array([pool["lid"].max() + 2, pool["nmp"].max() + 2])
    rho = density_at(pool["lid"], pool["nmp"], lid_e, nmp_e, 24, lo, hi)
    inv = 1.0 / (rho + 1e-6)
    inv = inv / (inv.max() + 1e-12)
    last_sc = last_frontier_scores(pool["lid"], pool["nmp"], lid_e, nmp_e)
    d_closed = np.hypot(lid_e - refs["closed_lid"], nmp_e - refs["closed_nmp"])
    ...
    for rank, idx in enumerate(picked):
        ...
        records.append(rec)
        log(f"  seed {rank:02d}  {seg['name']}  t={rec['t_ps']:.1f}  LID={rec['lid']:.1f} NMP={rec['nmp']:.1f}  score={rec['score']:.3f}")
    return records
...
def build_windows(segments, window, horizon, stride) -> dict:
    ...
    for si, seg in enumerate(segments):
        ...
        Xs.append(starts)
    if not extra:
        raise AdkError("no windows; raise --init-ns or lower --window-ps")
    return {
...
def run_short_round(spec, records, round_id: int, nt: int, gpu: bool, force: bool) -> list:
    ...
    for rec in records:
        ...
        if xtc.exists() and (job / "md_short.gro").exists() and dest.exists() and not force:
            ...
            if xtc.exists():
                extract_angles(tpr, xtc, dest, analysis_round / f"seed_{rank:02d}_scratch")
        if dest.exists():
...
def run_one_method(args, method: str, tag: str, nt: int, gpu: bool) -> dict:
    ...
    for rid in range(1, n_rounds + 1):
        ...
        log(f"  windows={len(pack['lid_end'])}")
        ...
        if seeds_json.exists() and not args.force:
            ...
            records = select_and_dump(
                spec, pack, method, args.n_seeds, args.min_deg, segments,
...
def parse_args():
    p = argparse.ArgumentParser(description="AdK 4-method adaptive sampling")
    ...
    p.add_argument("--tag-prefix", default="adk")
    ...
    p.add_argument("--min-deg", type=float, default=4.0)
    p.add_argument("--force", action="store_true")
    return p.parse_args()


def method_tag(prefix: str, method: str, _seed: int = 0) -> str:
    # Prefix already carries the replicate, e.g. adk vs adk_s1 (see run_discover.sh).
...
def main() -> int:
    ...
    if unknown:
        raise AdkError(f"unknown methods {unknown}; allowed: {list(METHOD_TAGS)}")
    if args.cpu and args.gpu:
        raise AdkError("use only one of --cpu / --gpu")
    gpu = bool(args.gpu) if (args.cpu or args.gpu) else run_md.detect_gpu()
    log("STAGE AdK  methods=" + ",".join(args.methods))
    if args.check:
        find_gmx()
        load_refs()
        for p in (OPEN_GRO, OPEN_DIR / "ions.gro", OPEN_DIR / "topol.top"):
            if not p.exists():
                raise AdkError(f"missing {p}; run bash scripts/prepare_adk.sh")
