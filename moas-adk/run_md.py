#!/usr/bin/env python3
"""Portable, resumable GROMACS runner for MOAS-AdK (apo adenylate kinase).

Works after the folder is copied anywhere. All paths are resolved from this
file's location.

  python3 run_md.py --check
  python3 run_md.py --system adk_open --length 0          # EM + NVT + NPT only
  python3 run_md.py --system adk_open --length 20 --gpu   # 20 ns open-start cMD
  python3 run_md.py --system adk_closed --length 0
...
SYSTEMS: Dict[str, SystemSpec] = {
    "adk_open": SystemSpec(
        key="adk_open",
        title="AdK apo open (4AKE) explicit water — production start",
    ...
    "adk_closed": SystemSpec(
        key="adk_closed",
        title="AdK apo closed (1AKE, ligand removed) explicit water — angle reference",
    ...
    if gro_in.exists():
        ...
        if natom < 500 and gpu:
            log(
                ...
                "that is normal. AdK water should load the GPU much more.",
                "WARN",
            )
    results = {}
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Self-checking, resumable GROMACS runner for MOAS-AdK.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Apo AdK only (no ligand). Default system: adk_open (4AKE).
adk_closed is the 1AKE apo reference box for LID/NMP angles.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if length and len(specs) > 1:
        log(
            ...
            "MOAS-AdK: start with --system adk_open --length 0, then --length 20",
            "WARN",
        )
    if args.check:
        ...
        if length:
            ...
            log(f"next: python3 run_md.py --system adk_open --length 0 --gpu --nt {nt}")
        return 0
