#!/usr/bin/env python3
"""Shared helpers for MOAS-MBP (no TAPS/CLN025 imports)."""
...
LOG_DIR = ROOT / "logs"
RUN_LOG = LOG_DIR / "mbp.log"


class MbpError(RuntimeError):
    pass
...
def find_gmx() -> str:
    ...
    if explicit:
        ...
        if found:
            return found
        raise MbpError(f"GMX={explicit} not found")
    for name in ("gmx", "gmx_mpi", "gmx_d"):
        found = shutil.which(name)
        if found:
            return found
    raise MbpError("gmx not in PATH")
...
def run_gmx(argv: Sequence[str], cwd: Path, stdin_text: str = "") -> subprocess.CompletedProcess:
    ...
    if proc.returncode != 0:
        ...
        raise MbpError(f"command failed ({proc.returncode}): {' '.join(str(x) for x in argv)}\n{tail}")
    return proc
...
def parse_xvg(path: Path) -> np.ndarray:
    ...
    if not rows:
        raise MbpError(f"no data in {path}")
...
def gro_ca(gro: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if len(ids) < 50:
        raise MbpError(f"found {len(ids)} CA atoms in {gro}")
