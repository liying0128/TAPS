#!/usr/bin/env python3
"""Portable, resumable GROMACS runner for MOAS (CLN025 only).
...
and not from any machine-specific absolute path.
  ...
  python3 run_md.py --system cln025_unfolded --length 100
  python3 run_md.py --system cln025_water --length 200
  python3 run_md.py --nt 16
  python3 run_md.py --cpu
  python3 run_md.py --force
"""
...
SYSTEMS: Dict[str, SystemSpec] = {
    "cln025_unfolded": SystemSpec(
        key="cln025_unfolded",
        title="Chignolin CLN025 water (unfolded start; MOAS main system)",
    ...
    "cln025_water": SystemSpec(
        key="cln025_water",
        title="Chignolin CLN025 water (native/folded start; cMD baseline)",
    ...
    if gro_in.exists():
        ...
        if natom < 500 and gpu:
            log(
                ...
                "that is normal. CLN025 water loads the GPU much more.",
                "WARN",
            )
    results = {}
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Self-checking, resumable GROMACS runner for MOAS (CLN025).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
MOAS uses Chignolin CLN025 only (no alanine dipeptide / tetrapeptide).

Default: cln025_unfolded
Native folded-start cMD: --system cln025_water
...
""",
    ...
    p.add_argument("--adk", action="store_true", help=argparse.SUPPRESS)
    p.add_argument(
        "--length",
        type=int,
        default=100,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if length and len(specs) > 1:
        log(
            ...
            "MOAS mainline: start with --system cln025_unfolded --length 100",
            "WARN",
        )
    if args.check:
